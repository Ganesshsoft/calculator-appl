...
class App extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (color && shape && clarity && userEnteredWeight) {
      let newHits = filter(originalHits, o => {
        return (
          Number.parseFloat(o._source.fromweight) <=
            Number.parseFloat(userEnteredWeight).toFixed(2) &&
          Number.parseFloat(userEnteredWeight).toFixed(2) <=
            Number.parseFloat(o._source.toweight)
        );
      });
      ...
      if (!newHits.length) {
        if (
          weightInclude &&
          Number.parseFloat(userEnteredWeight).toFixed(2) >= 10
        ) {
  ...
  handleWeightInputChange(val) {
    console.log("handleWeightInputChange", val);

    this.setState({
      userEnteredWeight: val,
  ...
  clearAll() {
    ...
    // this.state.numberCleave.setRawValue();
    // this.state.relCleave.setRawValue(0);
    // this.state.spCleave.setRawValue();
    // this.state.listCleave.setRawValue();
  }
  ...
  getRelToListWhenWeightActive() {
    let { hits, userEnteredTotalPc, userEnteredWeight, userVal } = this.state;
    console.log("getRelToListWhenWeightActive", userEnteredWeight);
    let relToList = "";
    if (!hits.length) {
      return relToList;
    }
    ...
    console.log("userEnteredWeightWeight", userEnteredWeight);
    let listPrice = hits[0]._source.ppc;
    console.log("listPriceWeight", listPrice);
    let sellPrice = userEnteredTotalPc / userEnteredWeight;
    console.log("sellPriceWeight", sellPrice);
    relToList = userVal;
    console.log("relTolist", relToList);
    return round(relToList, 2);
  }
  getTPWhenWeightActive() {
    let { hits, userEnteredWeight, userEnteredRelToList } = this.state;
    let totalPrice = 0;
    console.log("getTPWhenWeightActive", userEnteredWeight);
    if (!hits.length) {
      return totalPrice;
    }
    if (userEnteredWeight === "") {
  ...
  getSPWhenWeightActive() {
    let { userEnteredWeight, userEnteredRelToList, hits } = this.state;
    console.log("getTPWhenWeightActive", userEnteredWeight);
    let sellPrice = 0;
    if (!hits.length) {
      return sellPrice;
  ...
  render() {
    ...
    switch (activeInputField) {
      ...
      case "SELL_PRICE":
        ...
        break;
      case "WEIGHT":
        console.log("in case weight");
        relToList = this.getRelToListWhenWeightActive();
        relToList1 = isEmpty(relToList)
          ? this.getRelToListWhenWeightActive()
          : userVal;
        totalPrice = this.getTPWhenWeightActive();
        sellPrice = this.getSPWhenWeightActive();
        break;
      case "TOTAL_PRICE":
    ...
    if (!initialRendering && !hits.length && userEnteredWeight !== "") {
      ...
      resultHtml = (
        <fieldset className="form-fieldset">
          <legend className="form-legend">Pricing:</legend>
          ...
          <div className="outputSectionRow">
            <div className="outputColumns xs-device-set-margin">
              <h2 className="form-control-label">Relative to List</h2>
              {userEnteredWeight && color && shape && clarity ? (
                <div className="relToList">
                  <NumericInput
                    className="form-control relToListInput"
                    step={0.1}
                    precision={2}
                    value={relToList1}
                    format={myFormat}
                ...
                <div className="relToList">
                  <NumericInput
                    ...
                    disabled
                  />
                  <span className="suffix disable">%</span>
                </div>
              )}
            </div>
            <div className="outputColumns xs-device-set-margin">
              <h2 className="form-control-label">Rap List Per Carat</h2>
              {!checkBoxStatus ? (
                <Cleave
                  disabled
                  className="form-control"
                  value={listPrice}
                  onInit={this.listInit}
            ...
            <div className="outputColumns xs-device-set-margin">
              <h2 className="form-control-label">Price per Carat</h2>
      ...
      <div className="container">
        ...
        <form id="result-all">
          ...
          >
            <div id="input-area">
              <fieldset className="form-fieldset">
                <legend className="form-legend">Diamond Details:</legend>
                <div className="inputSectionRow">
                  <div className="inputColumns xs-device-set-margin">
                    ...
                    />
                    <label className="include-weight">
                      <input
                        type="checkbox"
                  ...
                  <div className="inputColumns">
                    <h2 className="form-control-label">Carat Weight</h2>
                    <NumericInput
                      className="form-control numeric-input carat-weight-input"
                      precision={2}
                      step={0.1}
                      strict={true}
                      value={weight}
            ...
            <div id="result-area">
              ...
              <SelectedFilters
                render={props => {
                  const { clearAllLabel, clearValues } = props;
